...
def RandomEquivalentPrimeIdeal(I, iso=False):
    """
    ...
    - if iso is True, also return the  left isomorphism from I to J
    """
    B = LLLReducedBasis(I)
...
def DecomposeQuaternionAlongBasis(gamma, basis, N):
    """
    Decompose a quaternion gamma along a given basis, with coefficients mod N
    Input:
    - gamma: a quaternion element
    - basis: a list of 4 quaternion elements forming a basis
    - N: modulus for the coefficients
    Output:
    - a vector of length 4 with coefficients c_i such that gamma = sum c_i*basis[i]
    """
    ZZN = Zmod(N)
    basis_vectors = [vector(ZZN, b.coefficient_tuple()) for b in basis]
    M = Matrix(ZZN, basis_vectors).transpose()
    v = vector(ZZN, gamma.coefficient_tuple())
