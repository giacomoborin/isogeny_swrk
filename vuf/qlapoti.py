...
def IdealToIsogeny(J, U, V, P=None, Q=None):
    ...
    theta_U0, theta_V0 = ec.EvalBasisPrecomp(d_inv*theta)

    # Compute the 2D isogeny
    K = ((params.U0, theta_U0), (params.V0, theta_V0))
    ...
    two_basis = (U, V)
    EI1, UI, VI = hd.EmbeddedIsogeny(phi, params.f, d1, basis=two_basis)
    UJ, VJ = ec.EvalBasisPrecomp(d_inv*theta_j, (UI, VI))

    if P and Q:
        # Evaluate it on arbitrary points (here on the twist)
        # Here we need to apply the scaling before
        PP = params.eval_end(theta_j/d1, P, params.N)
        QQ = params.eval_end(theta_j/d1, Q, params.N)
        twist_basis = (PP, QQ)
        EI, PJ, QJ = hd.EmbeddedIsogeny(phi, params.f, d1, basis=twist_basis)
        return EI1, UJ, VJ, PJ, QJ
    else:
        return EI1, UJ, VJ
