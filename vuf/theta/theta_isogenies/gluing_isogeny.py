...
class GluingThetaIsogeny(ThetaIsogeny):
    ...
    def __call__(self, P):
        ...
        try:
            ...
            E0 = self.T_shift[0].curve()
            E0t = P[0].curve()
            E0t_mont = E0t.montgomery_model(twisted=True, morphism=True)
            Fp2 = E0.base()
            Fp4, t = Fp2.extension(2, name='t').objgen()
            E04 = E0.base_extend(Fp4)
            E0t4 = E0t.base_extend(Fp4)
            # Mappa self.T_shift sul twist ora
            print('Gonna crash')
            breakpoint()
