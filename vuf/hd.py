...
def EmbeddedIsogeny(phi, n, d, basis=None):
    ...
    P.curve()._order = (params.p+1)**2*(params.p-1)**2
    k = P.order()
    tPQ = pari.ellweilpairing(phi.E1, P, Q, k)**d
    t1 = pari.ellweilpairing(phi.codomain()[0], phi_T[0], phi_S[0], k)
    t2 = pari.ellweilpairing(phi.codomain()[1], phi_T[1], phi_S[1], k)

    if tPQ == t1:
        return phi.codomain()[0], phi_T[0], phi_S[0]
    assert tPQ == t2, 'no side matches'
