...
class KummerLineIsogeny_Velu(KummerLineIsogeny_Generic):
    ...
    def __init__(self, domain, kernel, degree, check=True):
        ...
        if self._degree == 2:
            ...
            # these formula
            if not self._kernel.XZ()[0]:
                # We apply an isomorphism to the curve to move the kernel point away from (0:0:1)
                logger.info(f"The kernel is above (0:0:1), we need to apply an isomorphism...")
                A, C = domain.AC()
                alpha = (-A + (A**2-4*C**2).sqrt()) / (2*C)
                beta = (alpha**2-1).sqrt()
                beta = alpha/beta
                self._iso = [alpha, beta]
    ...
    def _apply_iso(self, P):
        """
        Apply the isomorphism that moves isogeny kernel away from (0:0:1) to the point P
        """
        assert self._iso, "No isomorphism to apply"
        alpha, beta = self._iso
        XP,ZP = P.XZ()
        XQ = beta*(ZP*alpha-XP)
        ZQ = alpha*ZP
        # Q = KummerPoint(XQ, ZQ)
    ...
    def _compute_codomain_constants_even(self):
        ...
        # Check if we need to apply an isomorphism first
        if self._iso: 
            XK,ZK = self._apply_iso(self._kernel)
        else:   
